#!/bin/bash
## Use this when testing on a small scale
out_dir="${@:1:1}"
max_parallel="${@:2:1}"
chunk_size="${@:3:1}"
hp_id="${@:4:1}"
...
#python $SIMS_TRUTHCATALOG_DIR/bin.src/write_gal_truth.py --help &
python $SIMS_TRUTHCATALOG_DIR/bin.src/write_gal_truth.py \
${out_dir} \
--healpixel ${hp_id} \
